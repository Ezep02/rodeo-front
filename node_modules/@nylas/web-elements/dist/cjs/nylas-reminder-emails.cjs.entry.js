...
const NylasReminderEmails = class {
    ...
    render() {
        ...
        return (index.h(index.Host, { key: 'b0be143161728aa82e29124cd7daca684481362e' }, index.h("div", { key: '5d272b3d2016c20cd38dc0b3a5e488bec54d1fb6', class: "cards-container" }, this.reminders.map((reminder, index$1) => {
            return reminder.type !== 'webhook' ? (index.h("div", { class: "nylas-reminder-email", part: "nre" }, index.h("div", { class: "header", part: "nre__header" }, index.h("div", null, index.h("h3", null, utils.instance.t('nylasReminderEmails.headerTitle', { organizerName: organizerName ? ` with ${organizerName} ` : '' })), index.h("p", null, utils.instance.t('nylasReminderEmails.headerSubTitle'))), !reminder.editing ? (index.h("button-component", { variant: 'basic', key: `edit-${index$1} `, clickHandler: event => {
                    event.preventDefault();
                    reminder.editing = true;
                    this.reminders = [...this.reminders];
                } }, index.h("edit-icon", { width: "16", height: "16" }), utils.instance.t('nylasReminderEmails.editButtonText'))) : (index.h("div", { class: "reminder-open-close-toggle" }, index.h("button-component", { variant: 'basic', class: "delete-btn", tooltip: "Delete reminder", key: `delete -${index$1} `, clickHandler: event => {
                    event.preventDefault();
                    this.deleteReminder(index$1);
                } }, index.h("trash-icon", { width: "16", height: "16" })), index.h("span", { class: `chevron ${reminder.editing ? 'open' : 'closed'}`, onClick: () => {
                    reminder.editing = !reminder.editing;
                    this.reminders = [...this.reminders];
                } }, index.h("chevron-icon", { width: "24", height: "24" }))))), reminder.editing ? (index.h("div", { class: "nylas-reminder-email__body", part: "nre__body" }, index.h("div", { class: "nylas-reminder-email__section" }, index.h("div", { class: "nylas-reminder-email__row" }, index.h("label", null, utils.instance.t('nylasReminderEmails.emailRecipients.label')), index.h("select-dropdown", { name: `recipient-${index$1}`, options: recipientOptions, defaultSelectedOption: recipientOptions.find(i => i.value === reminder.recipient) ?? recipientOptions[0], withSearch: false })), index.h("div", { class: "nylas-reminder-email__row" }, index.h("label", null, utils.instance.t('nylasReminderEmails.sendConfirmation.label'), index.h("span", { class: "label-icon" }, index.h("tooltip-component", null, index.h("info-icon", { slot: "tooltip-icon" }), index.h("span", { slot: "tooltip-content" }, utils.instance.t('nylasReminderEmails.sendConfirmation.tooltip.desc'))))), index.h("nylas-reminder-time", { name: `reminder-time-${index$1}`, eventReminderTimeMinutes: reminder.minutes_before_event, exportparts: "nrt, nrt__number-dropdown, nrt__number-dropdown-button, nrt__number-dropdown-content, nrt__period-dropdown, nrt__period-dropdown-button, nrt__period-dropdown-content, nrt__time-period-selector" })), index.h("div", { class: "nylas-reminder-email__block" }, index.h("label", null, utils.instance.t('nylasReminderEmails.emailSubject.label')), index.h("input-component", { name: `reminder-subject-${index$1}`, maxLength: 60, placeholder: utils.instance.t('nylasReminderEmails.emailSubject.placeholder', { organizerName: organizerName ? ` with ${organizerName}` : '' }), defaultValue: reminder?.email_subject || '' }))))) : (index.h("div", { class: "nylas-reminder-email__summary", part: "nre__summary" }, index.h("p", null, index.h("span", { class: "summary-icon" }, index.h("person-clipboard-icon", null)), recipientOptions.find(i => i.value === reminder.recipient)?.label), index.h("p", null, index.h("span", { class: "summary-icon" }, index.h("clock-icon", null)), this.formatReminderTime(reminder.minutes_before_event), " before the event"))))) : null;
        })), index.h("button-component", { key: '48bd0ab7e17afdbb185e41aa780218fcf18da0a2', class: "create-reminder", variant: 'basic', clickHandler: event => {
                event.preventDefault();
                this.addReminder();
            } }, index.h("plus-icon", { key: '3734f73c9916f80e2bbf771281c014ced6541fcd', width: "16", height: "16" }), utils.instance.t('nylasReminderEmails.newReminderButtonText'))));
    }
...
NylasReminderEmails.style = nylasReminderEmailsCss;

exports.nylas_reminder_emails = NylasReminderEmails;

//# sourceMappingURL=nylas-reminder-emails.cjs.entry.js.map