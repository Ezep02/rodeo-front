...
const nylasApiRequest = require('./nylas-api-request-7dd2b252.js');
...
class NylasSchedulerConfigAPIConnector {
    ...
    async getCalendars() {
        if (!this.nylasApiRequest) {
            console.warn('Nylas API Request is not initialized');
            return utils.dataResponse(null, {
    ...
    async getConfigurationById(configurationId, forceFetch = false) {
        ...
        if (!forceFetch) {
            ...
            if (existingConfig) {
                ...
                return utils.dataResponse(existingConfig);
            }
        }
        const [response, error] = await this.nylasApiRequest.request({
            method: 'GET',
...
class NylasSchedulerConfigConnector {
    ...
    init() {
        utils.debug('NylasSchedulerConfigConnector init');
    }
    ...
    resetStores() {
        utils.debug('NylasSchedulerConfigConnector resetStores');
        this.nylasSchedulerConfigStore.reset();
    }
    async logout() {
        utils.debug('NylasSchedulerConfigConnector logout');
        this.resetStores();
    }
}
...
class Storage {
    ...
    setPKCE(value) {
        const encrypt = utils.Buffer.from(value);
    ...
    async getPKCE() {
        try {
            ...
            if (pkce) {
                const b = utils.Buffer.from(pkce, "base64");
...
(function () {
  ...
  var NODE_JS = !root.JS_SHA256_NO_NODE_JS && typeof utils.process === 'object' && utils.process.versions && utils.process.versions.node;
  if (NODE_JS) {
    root = utils.commonjsGlobal;
  } else if (WEB_WORKER) {
    root = self;
  }
...
class NylasSessions {
    ...
    async getCodeChallege() {
        ...
        if (code_verifier) {
            ...
            let codeChallengeEncrypted = utils.Buffer.from(codeChallengeHashed).toString("base64");
...
const LoginRequired = ({ authenticationUrl }) => (index.h("div", { class: 'login-required', part: "nse__login-required login-component" },
    index.h("div", { class: 'card', part: "nse__login-required-card" },
        index.h("p", { part: "nse__login-required-text" }, "Log into your account to use the Scheduler Editor."),
        index.h("button", { class: "google-button", part: "nse__login-required-google-button", onClick: async (event) => {
            ...
            index.h("span", { class: "icon" },
                index.h("google-logo-icon", { width: "15", height: "15" })),
            "Continue with Google"),
        index.h("button", { class: "microsoft-button", part: "nse__login-required-microsoft-button", onClick: async (event) => {
                event.preventDefault();
            ...
            index.h("span", { class: "icon" },
                index.h("microsoft-logo-icon", { width: "15", height: "15" })),
            "Continue with Microsoft"),
        index.h("button", { class: "other-provider-button", part: "nse__login-required-other-provider-button", onClick: async (event) => {
                event.preventDefault();
            ...
            "View all providers ",
            index.h("chevron-icon", { width: "17", height: "17" })),
        index.h("small", { class: "disclaimer-text", part: "nse__login-required-disclaimer-text" }, "* You will be redirected to authenticate your account."))));
    ...
    return (index.h(index.Fragment, null,
        index.h("nylas-event-info", null,
            index.h("div", { slot: "inputs" },
                index.h("nylas-event-title", null),
                index.h("nylas-event-duration", null),
                index.h("nylas-event-location", { userProvider: currentUser.provider }))),
        index.h("nylas-booking-calendar-picker", { expressFlow: true, calendars: calendars, currentUser: currentUser, defaultBookingCalendar: defaultCalendarOption?.id }),
        index.h("nylas-scheduling-method", { exportparts: "nsm, nsm__header, nsm__input-label, nsm__dropdown, nsm__dropdown-button, nsm__dropdown-content" }),
...
const NylasSchedulerEditor = class {
    constructor(hostRef) {
        ...
        this.schedulerConfigCloseClicked = index.createEvent(this, "schedulerConfigCloseClicked", 7);
    ...
    async componentWillLoad() {
        utils.debug(`[nylas-scheduler-editor] componentWillLoad`);
        ...
        this.stores = {
            schedulerConfig: schedulerConfigStore.CreateNylasSchedulerConfigStore({
                ...
                selectedLanguage: this.defaultLanguage || constants.LANGUAGE_CODE.en,
            }),
        };
        if (!this.nylasApiRequest && this.nylasSessionsConfig) {
            ...
            this.nylasApiRequest = new nylasApiRequest.NylasIdentityRequestWrapper(nylasSession);
        }
    ...
    nylasFormDropdownChangedHandler(event) {
        ...
        if (name === 'language') {
            this.selectedLanguage = constants.LANGUAGE_CODE[value];
            utils.instance.changeLanguage(this.selectedLanguage);
    ...
    render() {
        ...
        return (index.h(index.Host, { key: '75f05f7ea3eb41f3610cd9a1d60196ee52f9e5ca' }, index.h("div", { key: '096bb8dbd32a0fc7ee3d37e7967d94d478410999', class: "scheduler-editor-header", part: "nse__header" }, index.h("h1", { key: '560bcc4419cafde8d07f49f9533ab45faaf4a8e0', class: "scheduler-editor-title", part: "nse__title" }, index.h("calendar-icon", { key: 'f4f6ae5e32f30199a5b8db1094fb1fcfb19c3a53', width: "18", height: "18" }), utils.instance.t('schedulerEditorTitle')), index.h("div", { key: '6150a5c179ebc92f8f076ed2cf9810d9c0ec301d', class: {
                language: true,
            }, part: "nse__language" }, index.h("select-dropdown", { key: '2aaf54341c6f44a605428d5c15ed6931d6623d2e', name: "language", options: languageOptions, withChevron: false, defaultSelectedOption: languageOptions.find(lang => lang.value == this.selectedLanguage), exportparts: "sd_dropdown: nse__language-dropdown, sd_dropdown-button: nse__language-drop-button, sd_dropdown-content: nse__language-drop-content, sd_dropdown_label: nse__language-drop-label" }, index.h("span", { key: '4299e684efc7a2024457cfa3ae0ec9ed1312dd6b', slot: "select-icon" }, index.h("translate-icon", { key: 'c6fd888c7c9b2e15b21f7693d8fe17ebe0f511d5', width: "20", height: "20" })))), index.h("button", { key: 'dc1ef92010c142ecdd6c1748c3fe669044af7d61', class: "scheduler-editor-close", title: "logout", onClick: e => this.handleSchedulerConfigCloseClicked(e), part: "nse__close-button" }, index.h("close-icon", { key: 'ae4dce679cbcfe0281337407d5303e7378b83669', width: "18", height: "18" }))), index.h("div", { key: '18dac5e2484e313ba211748be61c17138d31ebf0', class: "scheduler-editor-content", part: "nse__content" }, this.stores?.schedulerConfig?.state.currentUser && this.stores.schedulerConfig.state.action === null && (index.h("div", { class: "list-configurations", part: "nse__list-configurations" }, index.h("nylas-list-configurations", { exportparts: "nlc__create-new-cta" }))), this.stores?.schedulerConfig?.state.currentUser && this.stores.schedulerConfig.state.action === 'create' && (index.h("nylas-editor-tabs", { mode: "composable" }, index.h(ExpressFlow, { currentUser: this.stores?.schedulerConfig?.state.currentUser, calendars: this.stores?.schedulerConfig?.state?.calendars, selectedConfiguration: this.stores?.schedulerConfig?.state?.selectedConfiguration }))), this.stores?.schedulerConfig?.state.currentUser && this.stores.schedulerConfig.state.action === 'edit' && (index.h("nylas-editor-tabs", { key: selectedConfigId, mode: this.mode }, this.mode === 'composable' && index.h("slot", null), this.mode === 'app' && hasSlot && (index.h("div", { style: { display: 'none' }, slot: "custom-page-style-inputs", key: selectedConfigId }, index.h("slot", { name: "custom-page-style-inputs" }))))), !this.stores?.schedulerConfig?.state.currentUser && (index.h("slot", { name: "login-required" }, index.h(LoginRequired, { authenticationUrl: this.authenticationUrl }))))));
    }
...
exports.nylas_scheduler_editor = NylasSchedulerEditor;

//# sourceMappingURL=nylas-scheduler-editor.cjs.entry.js.map