...
class NylasAbstractProvider {
    ...
    async componentWillLoad(nylasConnector) {
        ...
        const registrations = childComponents.map(async (child) => {
            ...
            if (!component) {
                utils.debug(`[~${this.hostTagName}] (componentWillLoad) Component ${component.tagName} does not have a name. Skipping.`);
                return;
            }
            if (!component.registerNylasComponent) {
                utils.debug(`[~${this.hostTagName}] (componentWillLoad) Component ${component.tagName} is not a component that can be registered. Skipping.`, { component });
                return;
            }
    ...
    async componentDidLoad() {
        utils.debug(`[~${this.hostTagName}] (componentDidLoad) Did load`);
    }
    ...
    async registerComponentHandler(event) {
        ...
        if (!this.automaticComponentRegistration) {
            utils.debug(`~[${this.hostTagName}] (registerComponentHandler) Automatic component registration disabled. Skipping registration of ${event.detail.name}.`);
            return;
        }
    ...
    async unregisterComponentHandler(event) {
        event.stopPropagation();
        if (!this.automaticComponentRegistration) {
            utils.debug(`~[${this.hostTagName}] (unregisterComponentHandler) Automatic component registration disabled. Skipping unregistration of ${event.detail.name}.`);
            return;
        }
        this.unregisterComponent(event.detail);
    }
    async registerComponent(component) {
        utils.debug(`~[${this.hostTagName}] (registerComponent) Registering component ${component.name}.`);
        const elementId = component.element.dataset.nylasId;
        if (elementId && this.registeredComponents[elementId]) {
            utils.debug(`[${this.hostTagName}] (registerComponent) Component ${component.name} already registered. Skipping.`);
            return;
        }
        utils.debug(`[${this.hostTagName}] (registerComponent) Checking if "${component.element.localName}" component is defined`, { component });
        ...
        eventToProps?.forEach((customEventHandler, propKey) => {
            ...
            const handler = async (event) => {
                utils.debug(`[${this.hostTagName}] (registerComponent) Handling "${component.name}" prop "${String(propKey)}" event.`, { event });
                if (propKey in eventOverrides) {
                    utils.debug(`[${this.hostTagName}] (registerComponent) Found event override for "${String(propKey)}" event. Calling override.`);
            ...
            utils.debug(`[${this.hostTagName}] (registerComponent) Setting "${component.name}" event "${String(propKey)}" to automcially call NylasConnector method.`);
            const eventName = utils.formatEventName(propKey.toString());
            utils.debug(`[${this.hostTagName}] (registerComponent) Adding event listener for "${eventName}" on "${element.tagName}"`);
        ...
        stateToProps?.forEach((propKey, stateKey) => {
            ...
            if (!element.dataset.nylasId) {
                utils.debug(`[${this.hostTagName}] (registerComponent) Component "${component.name}" not registered. Skipping.`, { component });
                return;
            }
        ...
        localPropsToProp?.forEach((propKey, value) => {
            ...
            utils.debug(`[${this.hostTagName}] Setting "${component.name}" prop "${propKey.toString()}" to the value of "${value}" value.`, { value, mappedPropValue });
        });
        if (getStoresToProp) {
        ...
        if (typeof element.registerCallback === 'function') {
            utils.debug(`[${this.hostTagName}] (registerComponent) Calling registerCallback method for "${component.name}".`);
            await element.registerCallback();
        }
        utils.debug(`[${this.hostTagName}] (registerComponent) Component ${component.name} registered.`);
    }
    unregisterComponent(component) {
        utils.debug(`[${this.hostTagName}] Unregistering component ${component.name}.`);
        const elementId = component.element.dataset.nylasId;
        if (!elementId) {
            utils.debug(`[${this.hostTagName}] (unregisterComponent) Unregistering component ${component.name}.`);
            return;
        }
        if (this.registeredComponents[elementId]) {
            utils.debug(`[${this.hostTagName}] (unregisterComponent) Component ${component.name} not registered. Skipping.`);
            return;
        }
        ...
        eventListenerRegistrations?.forEach(([element, propKey, handler]) => {
            const eventName = utils.formatEventName(propKey.toString());
            utils.debug(`[${this.hostTagName}] (unregisterComponent) Removing event listener for "${eventName}" on "${element.tagName}"`);
    ...
    async setupStoreListeners() {
        ...
        Object.entries(this.stores).forEach(([storeName, store]) => {
            utils.debug(`[${self.hostTagName}] (setupStoreListeners) Listening for changes in "${storeName}" store.`);
            store.use({
                set(stateKey, newValue, oldValue) {
                    utils.debug(`[${self.hostTagName}] (setupStoreListeners) Store "${storeName}" state "${stateKey.toString()}" changed`, { newValue, oldValue });
                    if (newValue === oldValue) {
                        utils.debug(`[${self.hostTagName}] (setupStoreListeners) Store "${storeName}" state "${stateKey.toString()}" unchanged. Skipping.`);
                        return;
                    }
                    const registeredComponents = self.registeredComponents;
                    if (!registeredComponents) {
                        utils.debug(`[${self.hostTagName}] (setupStoreListeners) No registered components found. Skipping.`);
                        return;
                    }
                    const props = self.propStateRegistrations.get(storeName)?.get(stateKey.toString()) || [];
                    utils.debug(`[${self.hostTagName}] (setupStoreListeners) Found ${props.length} prop(s) registered for "${stateKey.toString()}" store.`, {
                        props,
                    ...
                    props.forEach(({ elementId, propKey }) => {
                        ...
                        if (!component) {
                            utils.debug(`[${self.hostTagName}] (setupStoreListeners) Component "${elementId}" not found. Skipping.`, { component });
                            return;
                        }
                        ...
                        utils.debug(`[${self.hostTagName}] (setupStoreListeners) Setting "${component.name}" prop "${propKey.toString()}" to "${stateKey.toString()}" value.`, { stateValue });
                    });
                },
            });
        });
...
const Xr = (t) => t !== null && typeof t == "object" && "error" in t;
async function ln(t) {
...
const I = typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : typeof window < "u" ? window : utils.global, B = Object.keys, M = Array.isArray;
function U(t, e) {
...
//# sourceMappingURL=index.es-963b294b.js.map