import { i as instance, b as addDaysToCurrentDate, a as debug } from './utils-14cfef23.js';
...
class NylaSchedulerAPIConnector {
    ...
    async bookTimeslot(data) {
        ...
        if (!data && !bookingInfo) {
            return { error: this.errors.component(instance.t('createBookingErrorTitle')).no_booking_info() };
        }
    ...
    async getUISettings() {
        ...
        if ('error' in response) {
            ...
            const errorType = response.error?.type;
            let error = response.error;
            if (errorType && errorType in this.errors.api(instance.t('getUISettingErrorTitle'))) {
                error = this.errors.api(instance.t('getUISettingErrorTitle'))[errorType](error?.message || error?.title || 'Something went wrong');
            }
            return { error };
        }
        if ('data' in response) {
    ...
    async getAvailability(startTime = 0, endTime = 0) {
        ...
        if (endTime && endTime < nowTime / 1000) {
            this.schedulerStore.set('isLoading', false);
            const error = this.errors.component(instance.t('getAvailabilityErrorTitle')).endtime_not_in_future();
            return { error };
        }
        if (!startTime) {
        ...
        if ('error' in response) {
            ...
            const errorType = response.error?.type;
            let error = response.error;
            if (errorType && errorType in this.errors.api(instance.t('getAvailabilityErrorTitle'))) {
                const errorMessage = this.getErrorMessage(error);
                error = this.errors.api(instance.t('getAvailabilityErrorTitle'))[errorType](errorMessage);
            }
            return { error };
        }
        if ('data' in response) {
    ...
    async cancelBooking(bookingId, reason) {
        this.schedulerStore.set('isLoading', true);
        if (!bookingId) {
            return { error: this.errors.component(instance.t('cancelBookingErrorTitle')).no_booking_id() };
        }
        const configIdParam = !this.sessionId && this.configId
            ? `?configuration_id=${this.configId}`
            : !this.sessionId && this.slug && this.clientId
        ...
        if ('error' in response) {
            ...
            const errorType = response.error?.type;
            let error = response.error;
            if (errorType && errorType in this.errors.api(instance.t('cancelBookingErrorTitle'))) {
                const errorMessage = this.getErrorMessage(error);
                error = this.errors.api(instance.t('cancelBookingErrorTitle'))[errorType](errorMessage);
            }
            return { error };
        }
    ...
    async rescheduleBooking(bookingId, data) {
        this.schedulerStore.set('isLoading', true);
        if (!bookingId) {
            return { error: this.errors.component(instance.t('rescheduleBookingErrorTitle')).no_booking_id() };
        }
        const apiErrors = this.errors.api(instance.t('rescheduleBookingErrorTitle'));
        const componentErrors = this.errors.component(instance.t('rescheduleBookingErrorTitle'));
    ...
    async updateBooking(payload) {
        ...
        const errorTitle = status === 'confirmed' ? instance.t('confirmBookingErrorTitle') : instance.t('rejectBookingErrorTitle');
        if (!bookingId) {
...
//# sourceMappingURL=scheduler-store-38a2b97c.js.map