...
describe('nylas-booked-event-card', () => {
    ...
    it('emits cancelBookingButtonClicked event on cancel button click', async () => {
        ...
        const cancelBtn = page?.root?.shadowRoot?.querySelector('[part="nbec__button-outline nbec__cancel-cta"]');
    ...
    it('emits rescheduleButtonClicked event on reschedule button click', async () => {
        ...
        const rescheduleBtn = page?.root?.shadowRoot?.querySelector('[part="nbec__button-outline nbec__reschedule-cta"]');
        const spyEvent = jest.fn();
...
describe('nylas-booked-event-card timezone', () => {
    it('renders the correct event time with the selected timezone', async () => {
        const page = await newSpecPage({
            components: [NylasBookedEventCard],
            template: () => h("nylas-booked-event-card", { eventInfo: eventInfo, selectedTimeslot: selectedTimeslot, selectedTimezone: 'America/Vancouver' }),
        });
        if (!page.root) {
            throw new Error('Component did not render.');
        }
        await page.waitForChanges();
        expect(page.root).toMatchSnapshot();
        const eventTime = page?.root?.shadowRoot?.querySelector('.booking-date-time p');
        expect(eventTime?.textContent).toContain('07:00 AM - 08:00 AM');
    });
});
