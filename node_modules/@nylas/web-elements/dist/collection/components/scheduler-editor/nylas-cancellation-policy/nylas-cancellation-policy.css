...
.nylas-cancellation-policy label {
  display: flex;
  align-items: center;
...
.nylas-cancellation-policy label span.label-icon tooltip-component {
  display: flex;
}
.nylas-cancellation-policy textarea {
  padding: 12px 16px;
  border-width: 1;
  resize: vertical;
  border-radius: 8px;
...
.nylas-cancellation-policy textarea::-webkit-resizer {
  display: none;
}