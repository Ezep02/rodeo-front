...
type Props = {
  ...
  fetchMoreImg: () => void;
  nextCursor: string;
  selectedImg: CloudinaryImage[];
};

const CloudImgSelector: React.FC<Props> = ({
  img,
  onSelect,
  onClose,
  fetchMoreImg,
  ...
  const [localSelected, setLocalSelected] = useState<CloudinaryImage[]>(
    selectedImg || []
  );

  useEffect(() => {
    setLocalSelected(selectedImg || []);
  }, [selectedImg]);
  ...
  const toggleSelectImg = (image: CloudinaryImage) => {
    const exists = localSelected.find(
      (img) => img.public_id === image.public_id
    );
    if (exists) {
      setLocalSelected(
        localSelected.filter((img) => img.public_id !== image.public_id)
      );
    } else {
      setLocalSelected([...localSelected, image]);
    }
  };

  // determinar si la imagen esta seleccionada
  const isSelected = (image: CloudinaryImage) =>
    localSelected.some((img) => img.public_id === image.public_id);
      ...
      <div className="p-5 md:px-10 flex-1">
        ...
        <div className="sticky bottom-0 flex justify-end gap-2 p-4">
          ...
          >
            ...
            Seleccionar {localSelected.length} imagen
          </Button>
        </div>
      </div>
    </div>
