...
const PostDialog: React.FC<Props> = ({
  ...
  const { setPost } = useContext(DashboardContext)!;
  ...
  useEffect(() => {
    if (isEditMode && initialData) {
      ...
      if (initialData.preview_url) {
        setSelectedImg({
          secure_url: initialData.preview_url,
        } as CloudinaryImage);
      }
    }
  ...
  const [postErrorMsg, submitPostAction, isPending] = useActionState(
    async (_state: void | null, data: FormValues) => {
      ...
      const postPayload: Omit<
        Post,
        "id" | "user_id" | "created_at" | "updated_at" | "is_published"
      > = {
        title: data.title,
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <DialogDescription className="text-start">
                  Agrega imágenes, crea descripciones llamativas y publica
                  contenido. Ideal para mostrar cortes, logros y novedades.
                </DialogDescription>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-zinc-500 mb-2">
                    Selecciona la imagen que se mostrará en la publicación
                  </p>
                </div>
