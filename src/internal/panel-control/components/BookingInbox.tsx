        ...
        {Array.isArray(inboxAppointment) && inboxAppointment.length > 0 ? (
          <ul className="flex flex-col gap-1">
            {inboxAppointment.map((sch, i) => {
              const time = new Date(sch.start).toLocaleTimeString("es-AR", {
                hour: "2-digit",
                minute: "2-digit",
                hour12: true,
              });
              const date = new Date(sch.start).toLocaleDateString("es-AR", {
                day: "numeric",
                month: "long",
              });
              ...
              return (
                ...
                >
                  ...
                  {/* --- Mensaje descriptivo --- */}
                  <div className="bg-white/60 rounded-2xl px-4 py-3 text-sm text-zinc-700">
                    <p className="leading-snug">
                      <span className="font-medium text-zinc-900">
                        Pago recibido ({sch.type})
                      </span>
                    </p>

                    <p className="mt-1 text-xs text-zinc-500">
                      Solicita confirmación del turno para el{" "}
                  ...
                  <div className="flex gap-1.5 pt-1">
                    ...
                    <ConfirmDialog
                      ...
                      trigger={
                        <button className="items-center rounded-full px-3 py-1.5 flex gap-1 bg-zinc-100 hover:bg-zinc-200 text-zinc-700 border border-zinc-300 transition cursor-pointer active:scale-95 font-medium text-sm">
                          Rechazar solicitud
                        </button>
                      }
                    />
